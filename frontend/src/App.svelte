...
<div>
	<Row>
      ...
      <ToastHeader>
				<Button info size="sm" style="margin-right: 6px">{ post.tag }</Button>
				<Label>{ formatDate(post.date) }</Label>
				<ButtonGroup style="position: absolute; right: 28px;">
				{#if (!post.isEdited)}
				<Button color="primary" size="sm" on:click={removePost({ post })} class="editbutton">Edit</Button>
				{/if}
				{#if (post.isEdited)}
				<Button color="primary" size="sm" on:click={removePost({ post })} class="editbutton">Update</Button>
				{/if}
				<Button color="danger" size="sm" on:click={removePost({ post })} class="delbutton">Delete</Button>
				</ButtonGroup>
			</ToastHeader>
      <ToastBody>
